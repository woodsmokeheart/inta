  ...
  const checkTelegramMembership = async (taskId) => {
    try {
      ...
      if (
        ...
        setCurrentError({
          [taskId]: `Verification failed, Join the channel to verify.`,
        });
      }
    } catch (error) {
      console.error("Error verifying Telegram membership:", error);
      setCurrentError({
        [taskId]: `Verification failed, Could not verify Telegram membership.`,
      });
    }
  };
      ...
      <Animate>
        <div className="w-full pt-1 justify-center flex-col space-y-6 px-5">
          <div className="w-full flex justify-between">
            ...
            <NavLink
              to="/checkin"
              className="font-medium text-[15px] text-[#C7B292] flex items-center justify-center space-x-1 relative"
            >
              <span className="w-[10px] h-[10px] animate-pulse bg-green-500 rounded-full flex items-center" />
              <span className="">Daily Checkin</span>
          ...
          <div className="w-full flex items-center justify-between border-cards border-[1px] p-1 rounded-[8px]">
            <div
              ...
              }  rounded-[6px] text-primary py-[10px] px-3 w-[33%] flex space-x-1 justify-center text-center text-[14px] font-semibold items-center`}
            >
            ...
            <div
              ...
              }  rounded-[6px] text-primary py-[10px] px-3 w-[33%] space-x-1 font-semibold text-[14px] flex justify-center text-center items-center`}
            >
              <PiYoutubeLogo size={16} className="" />
              <span className="relative">
                Videos
                <span className="absolute h-[8px] w-[8px] bg-red-500 animate-pulse rounded-full -right-3 top-0"></span>
              </span>
            </div>
          ...
          <div
            id="refer"
            className="w-full h-[80vh] !mt-3 scroller rounded-[10px] overflow-y-auto pt-2 pb-[250px]"
          >
            {/* tasks */}
            <div
              className={`${
            ...
            >
              <div className={`px-1`}>
                ...
                <p className="text-[14px] text-[#c6c6c6] leading-[24px] pr-8">
                  Perform social tasks to earn more INT tokens and stay updated!
                </p>
              </div>
                  ...
                  <div class="flex h-full w-full flex-col bg-[#2d2d2d] justify-center rounded-[24px] py-4 pl-12 pr-4 relative">
                    ...
                    <div className="flex w-full flex-col justify-between h-full space-y-2">
                      ...
                      <span className="flex text-primary items-center w-fit space-x-1 text-[14px] font-semibold">
                        <span
                          className={`w-[10px] h-[10px] bg-btn4 rounded-full flex items-center`}
                        ></span>
                        ...
                        {!completedTasks.includes(task.id) && (
                          <>
                            ...
                            <button
                              ...
                              className={`w-fit py-[6px] px-4 font-medium rounded-[6px] ${
                                countdowns[task.id]
                                  ? "hidden"
                          ...
                          <>
                            ...
                            !completedTasks.includes(task.id) ? (
                              ...
                              currentError[task.id] && (
                                <span
                                  className={`text-accent pt-2 text-xs w-full`}
                                >
                                  {currentError[task.id]}
                                </span>
                              )
